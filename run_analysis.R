...
train_x <- read.table("UCI HAR Dataset/train/X_train.txt",
                      header = FALSE,
                      colClasses="numeric",
                      col.names=features_desc);
train_x <- subset(train_x, select=mean_std_features)
XActivity <- read.table("UCI HAR Dataset/train/y_train.txt",
                        header = FALSE,
                        col.names=c("Activity"));
XSubject <- read.table("UCI HAR Dataset/train/subject_train.txt",
                       header = FALSE,
                       colClasses="numeric",
                       col.names=c("Subject"));
...
XActivity <- read.table("UCI HAR Dataset/test/y_test.txt",
                        header = FALSE,
                        col.names=c("Activity"));
XSubject <- read.table("UCI HAR Dataset/test/subject_test.txt",
                       header = FALSE,
                       colClasses="numeric",
                       col.names=c("Subject"));
...
ActivityName <- activities$name[mapvalues(data$Activity,
                    activities$id,
                    activities$name,
                    warn_missing = FALSE)]
